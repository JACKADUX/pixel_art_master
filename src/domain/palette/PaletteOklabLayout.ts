export const PALETTE_OKLAB_MAP_MAX_COLORS = 256;

export interface PaletteOklabCircleInput {
  id: string;
  hue: number;
  lightness: number;
}

export interface PaletteOklabCircleLayout {
  id: string;
  x: number;
  y: number;
  radius: number;
...
export function computePaletteOklabLayout(
  inputs: PaletteOklabCircleInput[],
  width: number,
  height: number,
): PaletteOklabCircleLayout[] {
