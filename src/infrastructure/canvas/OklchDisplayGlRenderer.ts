...
import { oklchLightnessFromRgb } from "@/domain/color/CanvasDisplayMode";
import {
  OKLCH_DISPLAY_FRAGMENT_SHADER,
  OKLCH_DISPLAY_VERTEX_SHADER,
} from "./oklchDisplay.glsl";
...
function createProgram(gl: WebGL2RenderingContext): WebGLProgram {
  const vert = compileShader(gl, gl.VERTEX_SHADER, OKLCH_DISPLAY_VERTEX_SHADER);
  const frag = compileShader(gl, gl.FRAGMENT_SHADER, OKLCH_DISPLAY_FRAGMENT_SHADER);
...
function renderWithCpuFallback(
  ...
  if (mode === "oklchLightness") {
    const { data } = imageData;
    for (let i = 0; i < data.length; i += 4) {
      const l = oklchLightnessFromRgb(data[i]!, data[i + 1]!, data[i + 2]!);
...
export class OklchDisplayGlRenderer {
  ...
  render(
    ...
    gl.uniform1i(this.modeLocation, mode === "oklchLightness" ? 1 : 0);
  ...
  initCanvas(canvas: HTMLCanvasElement): boolean {
    ...
    if (!gl) {
      console.warn("OklchDisplayGlRenderer: WebGL2 unavailable, using CPU fallback");
    ...
    try {
      ...
      console.warn("OklchDisplayGlRenderer: init failed, using CPU fallback", err);
      this.useCpuFallback = true;
      return false;
    }
  }
...
export function blitWithDisplayMode(
  renderer: OklchDisplayGlRenderer,
